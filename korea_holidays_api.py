"""
Korea Public Holidays API  v3.0
==================================
Railway 배포용 — 시작 시 자동 동기화
"""
...
HOLIDAY_NAMES = {
    "신정": "New Year's Day", "설날": "Lunar New Year's Day",
    "설날 전날": "Lunar New Year Eve", "설날 다음날": "Lunar New Year Holiday",
    "삼일절": "Independence Movement Day", "어린이날": "Children's Day",
    "부처님오신날": "Buddha's Birthday", "석가탄신일": "Buddha's Birthday",
    "현충일": "Memorial Day", "광복절": "Liberation Day",
    "추석": "Chuseok (Korean Thanksgiving)", "추석 전날": "Chuseok Eve",
    "추석 다음날": "Chuseok Holiday", "개천절": "National Foundation Day",
    "한글날": "Hangul Proclamation Day", "기독탄신일": "Christmas Day",
    "크리스마스": "Christmas Day", "대체냵휄일": "Substitute Holiday",
    "1월1일": "New Year's Day",
}
...
def sync(years=None):
    if not SERVICE_KEY:
        print("⚠️ DATA_GO_KR_KEY 없음 — 동기화 건녈뜀")
        return False
    if years is None:
        years = [2024, 2025, 2026, 2027]

    print(f"🔄 {years}년 공인흸일 수집 중...")
    con = sqlite3.connect(DB_FILE)
    saved = 0
    ...
    for year in years:
        for month in range(1, 13):
            ...
            try:
                ...
                print(f"  ⚠️ {year}-{month:02d}: {e}"); continue
            ...
            for item in items:
                ...
                con.execute("""
                    ...
                    VALUES (?,?,?,?,?,?,?)
                """, (
                    date_id, int(date_id[:4]), int(date_id[4:6]), int(date_id[6:8]),
                    name_ko, translate(name_ko), item["isHoliday"],
                ))
                saved += 1
...
app = FastAPI(
    ...
    description=(
        ...
        "Source: Korea Astronomy and Space Science Institute via data.go.kr\n\n"
        "Covers 2024–2027. Includes substitute holidays."
    ),
    version="3.0.0",
)
...
def startup():
    """서버 시작 시 DB 없으면 자동 동기화"""
    init_db()
    con = sqlite3.connect(DB_FILE)
    count = con.execute("SELECT COUNT(*) FROM holidays").fetchone()[0]
    con.close()
    if count == 0:
        print("📦 DB 비어있음 — 자동 동기화 시작")
        sync()
    else:
        print(f"✅ DB 준비됨 — {count}건")
...
def check_date(date: str):
    """Check if a specific date is a public holiday (YYYYMMDD, e.g. `20260101`)"""
...
def get_year(year: int):
    """All public holidays in South Korea for a given year (e.g. `2026`)"""
    rows = _db().execute(
...
def get_month(year: int, month: int):
    """Public holidays for a specific month (year/month, e.g. `2026/1`)"""
    if not 1 <= month <= 12:
...
if __name__ == "__main__":
    ...
    init_db()
    if args.test:
        print("🔍 테스트..."); sync([2026]); sys.exit(0)
    if args.sync:
